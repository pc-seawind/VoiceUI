...
def _print_trial_result(result: TrialResult) -> None:
    ...
    if result.clarity_selected_device or result.clarity_error:
        print(
            f"  clarity: selected={result.clarity_selected_device or 'none'} "
            f"margin={result.clarity_margin:.3f} "
            f"applied={int(result.clarity_applied)} "
            f"engine={result.clarity_engine or 'none'} "
            f"error={result.clarity_error or 'none'}"
        )
    for label, metrics in result.devices.items():
        print(
